...
class BoardDetector():
    ...
    def __init__(self):
        ...
        self.trans_old = [0.437, 0.126, 0.18]
        self.rot_old = quaternion.from_euler_angles(-np.pi/2, 0, 0)
    ...
    def get_trajectories(self):
        self.trans_old = [0.437, 0.126, 0.17]
        self.rot_old = quaternion.from_euler_angles(-np.pi/2, 0, 0)
        ...
        p1y = [self.board_length*0.2, -self.clearance, self.clearance]
        p2y = [p1y[0], p1y[1], -self.board_height*0.7]
        p3y = [p2y[0], self.board_width/2, p2y[2]]
    ...
    def execute_touch(self):
        ...
        trans_old = [0.437, 0.12, 0.2]
        rot_old = quaternion.from_euler_angles(-np.pi/2, 0, 0)
        ...
        for i in range(len(self.trajx)):
            ...
            # if self.collided:
            #     break
            goal = self.point_quat_to_goal_ref_to_base(self.trajx[i], self.orix, trans_old, rot_old)
        ...
        self.coll_points_to_save.append(self.coll_points[-1])
        self.coll_oris_to_save.append(self.coll_oris[-1])
        goal = self.point_quat_to_goal_ref_to_base(goal, self.trans, self.rot)
        goal = self.point_quat_to_goal_new_box(goal, self.trans, self.rot)
        self.go_to_pose(goal)
    ...
    def box_pose_from_2_points(self, point1, rot1, point2, rot2):
        ...
        intersect = np.array(l1.intersection(l2))

        pose = PoseStamped()
        pose.pose.position.x = intersect[0,0]
        pose.pose.position.y = intersect[0,1]
        pose.pose.position.z = intersect[0,2]
        pose.pose.orientation.w = np.sqrt(1-quaternion1.z**2)#quaternion1.w
        pose.pose.orientation.x = 0.0#quaternion1.x
        pose.pose.orientation.y = 0.0#quaternion1.y
...
if __name__ == '__main__':
    ...
    if not offline:
        ...
        print('two collision poses are', detector.coll_points_to_save, detector.coll_oris_to_save)
        pose_box_new = detector.box_pose_from_2_points(detector.coll_points_to_save[0], detector.coll_points_to_save[1], 
                                                    detector.coll_oris_to_save[0], detector.coll_oris_to_save[1])
        print('new_pose is', pose_box_new)
        new_box_pose = rospy.Publisher('/new_box_pose', Pose, queue_size=0)
        new_box_pose.pub(pose_box_new)
        coll_points_ref = np.load('coll_points_ref.npy')
        coll_oris_ref = np.load('coll_oris_ref.npy')
        pose_box_ref = detector.box_pose_from_2_points(coll_oris_ref[0], coll_oris_ref[0], coll_points_ref[1], coll_oris_ref[1])
